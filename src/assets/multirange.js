  ...
  const MultiRange = function MultiRange( placeholderElm, settings ){
      settings = typeof settings == 'object' ? settings : {}; // make sure settings is an 'object'

      this.width = getComputedStyle(placeholderElm).width;
      this.settings = {
      ...
      getRangesHTML(){
        ...
        //      this.ranges.unshift(this.settings.min)
          ...
          ranges.forEach((range, i) => {
              ...
              const prevPos = (ranges[i - 1] - that.settings.min) / (that.delta) * 100;
             ...
             // range =  ranges[i+1] - range;
              rangesHTML += `<div data-idx="${i}" class="multiRange__range" \
                  ...
                  <div class="multiRange__handle">\
                      <div class="multiRange__handle__value">${this.settings.names[i - 1]}<br>${(range - prevPos).toFixed(1).replace('.0', '')}%</div>\
                  </div>\
              </div>`;
          })
      ...
      events : {
          ...
          callbacks : {
              onMouseDown : function(e){
                  ...
                  this.DOM.name = this.DOM.currentSlice.querySelector('.multiRange__handle__value').innerHTML;
              ...
              onMouseMove : function(e){
                  ...
                  const name = this.settings.names[+this.DOM.currentSlice.dataset.idx - 1];

                  // update the DOM
                  window.requestAnimationFrame(function(){
                      if( that.DOM.currentSlice ){
                          that.DOM.currentSlice.style.left = leftPrecentage + '%';
                          that.DOM.currentSliceValue.innerHTML = `${name}<br>${(value - prevSliceValue).toFixed(1).replace('.0', '')}%`;
                      }
                  })
                  // update "ranged" Array
                  this.ranges[this.DOM.currentSlice.dataset.idx] = +value.toFixed(1);
                  let offset =  +value.toFixed(1);

                  const nextIdx = +this.DOM.currentSlice.dataset.idx + 1;

                  const wrap = document.querySelector(`.multiRange__range[data-idx="${nextIdx}"] .multiRange__handle__value`);

                  wrap.innerHTML = this.settings.names[nextIdx - 1] + '<br>' + (this.ranges[nextIdx] - offset).toFixed(1).replace('.0', '') + '%';
